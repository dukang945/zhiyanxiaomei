  ...
  <div class="evaluationContent">
    ...
    <el-dialog title="产品管理" :visible.sync="productDialogVisible" width="60%">
      ...
      <el-table :data="productTableData" border style="width: 100%">
        <el-table-column type="index" label="序号" width="50" align="center"></el-table-column>
        <el-table-column prop="id" label="编号" width="100" align="center"></el-table-column>
        <el-table-column prop="beautyAppraisalId" label="知识ID" width="100" align="center"></el-table-column>
        <el-table-column
          prop="productId"
          label="产品ID"
          width="100"
    ...
    <el-dialog title="新增产品" :visible.sync="addProductDialogVisible" width="70%" append-to-body>
      <el-form :label-position="labelPosition" label-width="100px" :model="addproductForm" width="900px">
...
export default {
  ...
  methods: {
    ...
    handleAdd() {
      ...
      console.log(testObj);
      this.$axios
        .post("/management/admin/beauty-knowledge!save.action", paramsStr)
        .then(res => {
          this.getTableData(
            "/management/admin/beauty-knowledge!list.action",
            this.page,
            this.row
          );
    ...
    batchOnline() {
      if (this.checkedRowId) {
        this.$axios
          .post(
            `/management/admin/beauty-knowledge!batchOnLine.action?ids=${
              this.checkedRowId
            }&online=0`
          )
          .then(res => {
    ...
    batchOffline() {
      ...
      if (this.checkedRowId) {
        this.$axios
          .post(
            `/management/admin/beauty-knowledge!batchOffLine.action?ids=${
              this.checkedRowId
            }&online=1`
          )
          .then(res => {
    ...
    batchDelete() {
      this.$confirm("确定要进行批量删除么？").then(res => {
        if (this.checkedRowId) {
          this.$axios
            .post(
              `/management/admin/beauty-knowledge!batchDelete.action?ids=${
                this.checkedRowId
              }`
            )
            .then(res => {
    ...
    saveH5(row) {
      this.$axios
        .get(`/management/admin/beauty-knowledge!updateTime.action?id=${row.id}`)
        .then(res => {
          if (res.status == 200) {
    ...
    addLabel() {
      if (this.choosedLabelList.length > 0) {
        ...
        this.$axios
          .post(
            `/management/admin/beauty-knowledge!batchAddLabel.action?ids=${
              this.checkedRowId
            }`,
            idString
          )
    ...
    edit(index, row) {
      ...
      if (row.videoUrl) {
        ...
        console.log(this.editVideo, this.editBannerPicFileList);
      }
    },
    // 取消编辑
    cancelEdit() {
    ...
    handleEdit() {
      ...
      if (this.tempListImgUrl) {
        testObj.image = this.tempListImgUrl
      } else {
        testObj.image = "";
      }
      if (this.tempBannerImgUrl) {
        testObj.bannerImage = this.tempBannerImgUrl
      } else {
        testObj.bannerImage = "";
      }
      ...
      this.$axios
        .post(
          `/management/admin/beauty-knowledge!save.action?id=${
    ...
    deleteRow(index, row) {
      this.$confirm("确认删除？")
        .then(_ => {
          this.$axios
            .get(
              `/management/admin/beauty-knowledge!delete.action?id=${row.id}`
            )
            .then(res => {
    ...
    switchOnline(index) {
      ...
      this.$axios
        .post(
          `/management/admin/beauty-knowledge!online.action?id=${
    ...
    switchTop(index) {
      ...
      this.$axios
        .post(
          `/management/admin/beauty-knowledge!sticky.action?id=${
            this.tableData[index].id
          }&sticky=${status}`
        )
        .then(res => {
    ...
    updataSort(index, row) {
      this.$axios
        .post(
          `/management/admin/beauty-knowledge!updateTime.action?id=${row.id}`
        )
        .then(res => {
          if (res.status == 200) {
 ...
 // 回复评论（二级评论）
        ...
        this.$axios
          .post(
            `/management/admin/new-comment!save.action?beautyId=${
              this.commentRowId
            }&id=${this.commentId}&type=3&parentId=${this.parentCommentId?this.parentCommentId:this.commentId}`,
    ...
    getCommentData(page, rows, id) {
      this.$axios
        .post(`/management/admin/new-comment!list.action?beautyId=${id}&type=3`, this.$qs.stringify({page,rows}))
        .then(res => {
          if (res.data.total > 0) {
    ...
    getCommentData2(page, row, id) {
      this.$axios.get(`/management/admin/new-comment!getCommentListByCommentId.action?id=${this.commentId}&beautyId=${this.userId}&type=3`,{params: {
            page: page,
            rows: row
          }}).then(res=>{
        if(res.status==200){
