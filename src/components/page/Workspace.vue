<template>
  <div class="dataSpace">
    <el-button
      ...
      size="mini"
      plain
      @click="allDataYesterday('/management/admin/homepage!getSequentialData.action')"
    >前一天</el-button>
    <el-button
      type="primary"
      size="mini"
      plain
      @click="allDataYesterday('/management/admin/homepage!getSequentialDataSevenDay.action')"
    >近七天</el-button>
    <el-button
      type="primary"
      size="mini"
      plain
      @click="allDataYesterday('/management/admin/homepage!getSequentialDataThirtyDay.action')"
    >近一个月</el-button>
    <el-row
      class="topData"
      :gutter="30"
    ...
    >
      <el-col :span="6">
        <div
          class="cardItem"
          style="background: linear-gradient(to right, rgba(255, 255, 255,1), rgba(50, 208, 136,0.2))"
        >
          <div
            class="cardLeft"
            style="color: #32D088;"
          ...
          <div class="cardRight">
            ...
            <p class="text2">注册量</p>
            <p
              class="text3 numberDown"
              v-if="cardData.registeredTrend<0"
            >
              环比 - {{cardData.registeredTrend*(-1)}} %
              <i class="el-icon-caret-bottom"></i>
            </p>
            <p
              class="text3 numberUp"
              v-else
            >
              环比 + {{cardData.registeredTrend}} %
      ...
      <el-col :span="6">
        <div
          class="cardItem"
          style="background: linear-gradient(to right, rgba(255, 255, 255,1), rgba(251, 110, 81,0.2))"
        >
          <div
            class="cardLeft"
            style="color: #FB6E51;"
          ...
          <div class="cardRight">
            ...
            <p
              class="text3 numberDown"
              v-if="cardData.readTrend<0"
            >
              环比 - {{cardData.readTrend*(-1)}}%
              <i class="el-icon-caret-bottom"></i>
            </p>
            <p
              class="text3 numberUp"
      ...
      <el-col :span="6">
        <div
          class="cardItem"
          style="background: linear-gradient(to right, rgba(255, 255, 255,1), rgba(245, 178, 82,0.2))"
        >
          <div
            class="cardLeft"
            style="color: #F5B252;"
          ...
          <div class="cardRight">
            ...
            <p
              class="text3 numberDown"
              v-if="cardData.commentDataTrend<0"
            >
              环比 - {{cardData.commentDataTrend*(-1)}} %
              <i class="el-icon-caret-bottom"></i>
            </p>
            <p
              class="text3 numberUp"
      ...
      <el-col :span="6">
        <div
          class="cardItem totalNumber"
          style="background: linear-gradient(to right, rgba(50, 208, 136,0.2),rgba(50, 206, 228,0.2),rgba(251, 110, 81,0.2), rgba(245, 178, 82,0.2))"
        >
          <p>
            总注册量
            <span>{{cardData.registeredAll-11156}}</span>
          </p>
          <p>
            总阅读量
            <span>{{cardData.readAll}}</span>
          </p>
          <p>
            总评论量
            <span>{{cardData.commentAll}}</span>
          </p>
        </div>
      </el-col>
    </el-row>
      ...
      >
        ...
        <div class="timeSelect">
          ...
          <el-button
            ...
            size="mini"
            plain
            @click="checkUserYesterday"
          >前一天</el-button>
          <el-button
            type="primary"
            size="mini"
            plain
            @click="checkUserLastWeek"
          >近七天</el-button>
          <el-button
            type="primary"
            size="mini"
            plain
            @click="checkUserLastMonth"
          >近一个月</el-button>
          <el-button
            type="primary"
            size="mini"
            plain
            @click="checkUserAllTime"
          >所有时间</el-button>
        </div>
      </el-col>
    </el-row>
    ...
    <el-row
      class="chartData labelChart"
      style="margin: 30px 0"
    >
      <el-col :span="24">
        <div class="title">标签阅读量TOP10--{{labelType}}</div>
        <div
          ref="labelEchart"
          class="chart1"
        ></div>
        ...
        <div class="timeCompare">
          对比时间：
          <el-date-picker
            v-model="LabelDataTime2"
            type="daterange"
            size="mini"
            placeholder="选择某一天"
            value-format="yyyy-MM-dd"
            @change="chooseTimeForLabel2"
            unlink-panels
            range-separator="至"
            start-placeholder="开始日期"
            end-placeholder="结束日期"
      ...
      >
        <div class="articleSelect">
          ...
          <el-button
            ...
            size="mini"
            plain
            @click="getPageReadData"
          >查询</el-button>
        </div>
        <p class="tableHead">文章阅读量</p>
        <el-table
          :data="pageTableData"
          border
          style="width: 80%;margin: 0 auto;"
...
export default {
  ...
  methods: {
    //卡片数据
    allDataYesterday(url) {
      this.$axios.get(url).then(res => {
        // 格式化数据
        this.cardData.registeredData = res.data.registeredYesterday;
        if (res.data.registeredYesterday > 0) {
          this.cardData.registeredTrend = Math.round(
            ((res.data.registeredYesterday - res.data.registeredvVrgestern) /
              res.data.registeredYesterday) *
              100
          );
        } else {
        ...
        if (res.data.yesterdayRead > 0) {
          this.cardData.readTrend = Math.round(
            ((res.data.yesterdayRead - res.data.vorgesternRead) /
              res.data.yesterdayRead) *
              100
          );
        } else {
        ...
        if (res.data.vorgesternCommentNum > 0) {
          this.cardData.commentDataTrend = Math.round(
            ((res.data.yesterdayCommentNum - res.data.vorgesternCommentNum) /
              res.data.yesterdayCommentNum) *
              100
          );
        } else {
          this.cardData.commentDataTrend = -res.data.vorgesternCommentNum * 100;
        }
        if (url == "/management/admin/homepage!getSequentialData.action") {
    ...
    checkTeach() {
      ...
      if (this.labelTime) {
        this.getLabelReadData(1, 3, this.labelTime);
      } else {
    ...
    checkEvaluation() {
      ...
      if (this.labelTime) {
        this.getLabelReadData(2, 3, this.labelTime);
      } else {
    ...
    checkNew() {
      ...
      if (this.labelTime) {
        this.getLabelReadData(7, 3, this.labelTime);
      } else {
    ...
    chooseTimeForLabel2(value) {
      console.log(value, this.LabelDataTime2);
      this.getLabelReadData(
        this.choosedLabelType,
        "",
        this.LabelDataTime,
        value
    ...
    getUserOfData(time, date) {
      ...
      if (date != "" && time == "") {
        http = `/management/admin/homepage!userOfData.action?`;
      }
      if (http) {
        this.$axios
          .post(
            http,
            this.$qs.stringify({
              startTime: date ? date[1] : "",
              endDateTime: date ? date[0] : ""
            })
          )
          .then(res => {
            let data2 = [
    ...
    // 获取标签TOP10数据
    getLabelReadData(type, time, date, date2) {
      var httpLabel;
      if (type && time != "") {
        httpLabel = `/management/admin/homepage!labelReadNumData.action?labelType=${type}&selectType=${time}`;
      } else if (type && date != "") {
        httpLabel = `/management/admin/homepage!labelReadNumData.action?labelType=${type}`;
      } else if (type == "" && time != "") {
        httpLabel = `/management/admin/homepage!labelReadNumData.action?selectType=${time}`;
      } else if (type == "" && date != "") {
        httpLabel = `/management/admin/homepage!labelReadNumData.action`;
      }
      if (httpLabel) {
        if (!date2) {
          this.$axios
            ...
            .then(res => {
              this.$axios
                .post(
                  httpLabel,
                  this.$qs.stringify({
                    startTime: date2 ? date2[1] : "",
                    endDateTime: date2 ? date2[0] : ""
                  })
                )
                .then(res2 => {
                  let data1 = [
                  ...
                  switch (this.tagType) {
                    case 1:
                      res.data.forEach(item => {
                        data1.push([item.name, item.readNum]);
                      });
                      res2.data.forEach((item, index) => {
                        console.log(index, item);
                        data1[index + 1].push(item.readNum);
                      });
                      break;
                    case 2:
                      res.data.forEach(item => {
                        data1.push([item.name, item.enshrine]);
                      });
                      res2.data.forEach((item, index) => {
                        console.log(index, item);
                        data1[index + 1].push(item.enshrine);
                      });
                      break;
                    case 3:
                      res.data.forEach(item => {
                        data1.push([item.name, item.likeNum]);
                      });
                      res2.data.forEach((item, index) => {
                        console.log(index, item);
                        data1[index + 1].push(item.likeNum);
                      });
                      break;
                    case 4:
                      res.data.forEach(item => {
                        data1.push([item.name, item.commentNum]);
                      });
                      res2.data.forEach((item, index) => {
                        console.log(index, item);
                        data1[index + 1].push(item.commentNum);
                      });
                      break;
                    case 5:
                      res.data.forEach(item => {
                        data1.push([item.name, item.shareNum]);
                      });
                      res2.data.forEach((item, index) => {
                        console.log(index, item);
                        data1[index + 1].push(item.shareNum);
                      });
                      break;
                  }

                  this.series = [{ type: "bar" }, { type: "bar" }];
                  console.log(data1);
                  this.initChart1(data1);
                });
            });
    ...
    getPageReadData() {
      ...
      if (this.value2.length == 0) {
        this.$axios
          .post(
            "/management/admin/homepage!getBeautyDetailsTop10.action",
            this.$qs.stringify({
              page: this.page1,
              rows: this.rows1,
              startTime: this.value2 ? this.value2[0] : "",
              endDateTime: this.value2 ? this.value2[1] : ""
            })
          )
          .then(res => {
        ...
        this.$axios
          .post(
            `/management/admin/homepage!getBeautyDetailsTop10ByTime.action`,
            this.$qs.stringify({
              page: this.page1,
              rows: this.rows1,
              startTime: this.value2 ? this.value2[1] : "",
              endDateTime: this.value2 ? this.value2[0] : ""
            })
          )
          .then(res => {
            this.pageTableData = res.data;
    ...
    gettitleData() {
      this.$axios
        .post(
          "/management/admin/homepage!getZtTop10ByTime.action",
          this.$qs.stringify({
            startTime: this.value1 ? this.value1[0] : "",
            endDateTime: this.value1 ? this.value1[1] : ""
          })
        )
        .then(res => {
          if (res.status == 200) {
    ...
    handleSizeChange(val) {
      this.rows1 = val;
      this.getPageReadData(this.page1, this.rows1);
    },
    handleCurrentChange(val) {
      this.page1 = val;
      this.getPageReadData(this.page1, this.rows);
    }
  },
  watch: {
    tagType(val) {
  ...
  mounted() {
    // 获取卡片数据
    this.allDataYesterday(
      "/management/admin/homepage!getSequentialData.action"
    );
    // 获取柱状图默认数据
    this.getLabelReadData("", "3", "");
    // 获取人员组成默认数据
    ...
    // 获取其他用户信息
    this.$axios
      .get("/management/admin/homepage!userOfData1.action")
      .then(res => {
        if (res.data) {
          // 格式化数据
          let data3 = [
    ...
    // 获取专题top10信息
    this.$axios
      .get("/management/admin/homepage!getZtTop10.action")
      .then(res => {
        this.objectTableData = res.data;
      });
    // 获取文章阅读量
