<template>
  <div class="evaluationContent">
      ...
      <el-col :span="18">
        <div class="btnGroup">
          ...
          <el-dialog
            ...
            :visible="AddVisible"
            width="60%"
            @close="AddVisible=false;cancelAdd()"
            @open="open"
          >
      ...
      >新增</el-button>
              ...
              <el-form-item label="banner图片">
                <el-upload
                  ...
                  :data="imgData"
                  :before-upload="beforeUpload"
                  :on-success="uploadBannerImgSuccess"
                  :on-preview="handlePictureCardPreview"
                  :limit="1"
    ...
    <el-radio-group v-model="formLabelAdd.orientation">
      ...
      <el-col :span="6">
        <el-form :inline="true" :model="searchForm" class="right-search" @submit.native.prevent>
          <el-form-item>
            <el-input
    ...
    <el-row :gutter="10">
      ...
      <el-col :span="22">
        ...
        >
          ...
          <el-table-column prop="id" label="id" width="50" align="center"></el-table-column>
          <el-table-column prop="name" label="标题" align="center" :show-overflow-tooltip="true" width='300'></el-table-column>
          <el-table-column
            prop="labelName"
            label="标签"
            width="200"
          ...
          <el-table-column
            ...
            :show-overflow-tooltip="true"
          ></el-table-column>
          <el-table-column label="操作" fixed="right" align="center" width="660">
            <template slot-scope="scope">
              <el-button
                size="small"
                type="primary"
              ...
              >预览</el-button>
              <el-button size="small" type="primary" @click="saveH5(scope.row)">更新时间</el-button>
              <el-button
                size="small"
                type="primary"
                @click="switchOnline(scope.$index, scope.row)"
                class="el-icon-sort"
              >{{tableData[scope.$index].online==0?'下线':'上线'}}</el-button>
              ...
              <el-button
                size="small"
                type="primary"
                @click="checkProduct(scope.$index, scope.row)"
              >产品管理</el-button>
              <el-button
                size="small"
                type="primary"
                @click="checkComment(scope.$index, scope.row)"
              >评论管理</el-button>
            </template>
          </el-table-column>
        </el-table>
      ...
      >新增</el-button>
        ...
        <el-form-item label="banner图片">
          <el-upload
            ...
            :on-preview="handlePictureCardPreview"
            :limit="1"
            :file-list="editBannerPicFileList"
            list-type="picture"
          >
            <el-button size="small" type="primary">点击上传</el-button>
            <div slot="tip" class="el-upload__tip">只能上传jpg/png文件，且不超过500kb</div>
      ...
      <el-form :label-position="labelPosition" label-width="100px" :model="addCommentForm">
        <el-form-item label="回复用户">
          <el-select v-model="addCommentForm.userId" placeholder="请选择用户" style="width: 100%;">
            ...
            <div class="noMargin">
              <Pagination
                :totalNum="userTotalNum"
                @change_Page="changeUserPage"
                @change_Size="changeUserSize"
              ></Pagination>
            </div>
          </el-select>
        </el-form-item>
    ...
    <el-dialog title="评论管理" :visible.sync="checkCommentDialogVisible" width="60%">
      ...
      <Pagination
        :totalNum="commentTotalNum"
        @change_Page="changeCommentPage"
        @change_Size="changeCommentSize"
      ></Pagination>
    </el-dialog>
     <!-- 查看二级评论 -->
    <el-dialog title="评论管理" :visible.sync="checkComment2DialogVisible" width="80%">
      <el-button type="primary" @click="addComment" size="small" style="margin-bottom: 20px;">新增评论</el-button>
      <el-table :data="comment2TableData" border style="width: 100%">
        <el-table-column type="index" label="序号" width="50" align="center"></el-table-column>
        <el-table-column prop="id" label="编号" width="100" align="center"></el-table-column>
        <el-table-column prop="content" label="评论内容" align="center"></el-table-column>
        <el-table-column prop="time" label="评论时间" width="150" align="center"></el-table-column>
        <el-table-column prop="nickName" label="评论用户" width="100" align="center"></el-table-column>
        <el-table-column prop="parentNickName" label="回复的用户" width="100" align="center"></el-table-column>
        <el-table-column prop="greatNumber" label="点赞数" width="100" align="center"></el-table-column>
    ...
    <el-dialog title="产品管理" :visible.sync="productDialogVisible" width="60%">
      ...
      <el-table :data="productTableData" border style="width: 100%">
        <el-table-column type="index" label="序号" width="50" align="center"></el-table-column>
        <el-table-column prop="id" label="编号" width="100" align="center"></el-table-column>
        <el-table-column prop="beautyAppraisalId" label="测评ID" width="100" align="center"></el-table-column>
        <el-table-column
          prop="productId"
          label="产品ID"
          width="100"
          align="center"
          :formatter="getProductIdStr"
          :show-overflow-tooltip="true"
        ></el-table-column>
        <el-table-column
          prop="productColor"
          label="色号名称"
          align="center"
          :formatter="getProductColorStr"
          :show-overflow-tooltip="true"
        ></el-table-column>
        <el-table-column label="操作" width="200" align="center">
          <template slot-scope="scope">
            <el-button
              @click.native.prevent="deleteProduct(scope.$index, scope.row)"
              size="small"
              type="danger"
              class="el-icon-delete"
            >删除</el-button>
      ...
      <Pagination
        :totalNum="productTotalNum"
        @change_Page="changeProductPage"
        @change_Size="changeProductSize"
      ></Pagination>
    </el-dialog>
    <!-- 新增产品弹框 -->
    <el-dialog title="新增产品" :visible.sync="addProductDialogVisible" width="70%" append-to-body >
      <el-form :label-position="labelPosition" label-width="100px" :model="addproductForm" class="createdPro">
        <el-form-item label="选择色号" width="900px">
          <el-select
            v-model="addproductForm.productColor"
            multiple
            placeholder="请选择色号"
            filterable
            :filter-method="(q)=>{
          ...
          >
            ...
            <el-option
              v-for="item in productColorOptions"
              :key="item.id"
            ...
            <div class="noMargin">
              <Pagination
                :totalNum="colorTotalNum"
                @change_Page="changeColorPage"
                @change_Size="changeColorSize"
              ></Pagination>
            </div>
          </el-select>
        </el-form-item>
    ...
    <el-dialog title="编辑产品" :visible.sync="editProductDialogVisible" width="50%" append-to-body>
      <el-form :label-position="labelPosition" label-width="100px" :model="editProductForm">
        <el-form-item label="选择色号">
          <el-select
            v-model="editProductForm.productColor"
            multiple
            placeholder="请选择色号"
            filterable
            remote
...
export default {
  ...
  methods: {
   // 获取博主来源列表
    ...
    handleAdd() {
      ...
      console.log(testObj);
      this.$axios
        .post("/management/admin/beauty-appraisal!save.action", paramsStr)
        .then(res => {
          this.getTableData(
            "/management/admin/beauty-appraisal!list.action",
            this.page,
            this.row
          );
    ...
    batchOnline() {
      if (this.checkedRowId) {
        this.$axios
          .post(
            `/management/admin/beauty-appraisal!batchOnLine.action?ids=${
              this.checkedRowId
            }&online=0`
          )
          .then(res => {
    ...
    batchOffline() {
      ...
      if (this.checkedRowId) {
        this.$axios
          .post(
            `/management/admin/beauty-appraisal!batchOffLine.action?ids=${
              this.checkedRowId
            }&online=1`
          )
          .then(res => {
    ...
    saveH5(row) {
      this.$axios
        .get(`/management/admin/beauty-appraisal!updateTime.action?id=${row.id}`)
        .then(res => {
          if (res.status == 200) {
    ...
    addLabel() {
      if (this.choosedLabelList.length > 0) {
        ...
        this.$axios
          .post(
            `/management/admin/beauty-appraisal!batchAddLabel.action?ids=${
              this.checkedRowId
            }`,
            idString
          )
    ...
    batchDelete() {
      this.$confirm("确定要进行批量删除么？").then(res => {
        if (this.checkedRowId) {
          this.$axios
            .post(
              `/management/admin/beauty-appraisal!batchDelete.action?ids=${
                this.checkedRowId
              }`
            )
            .then(res => {
    ...
    handleEdit() {
      ...
      this.$axios
        .post(
          `/management/admin/beauty-appraisal!save.action?id=${
    ...
    deleteRow(index, row) {
      this.$confirm("确认删除？")
        .then(_ => {
          this.$axios
            .get(
              `/management/admin/beauty-appraisal!delete.action?id=${row.id}`
            )
            .then(res => {
    ...
    switchOnline(index) {
      ...
      this.$axios
        .post(
          `/management/admin/beauty-appraisal!online.action?id=${
    ...
    switchTop(index) {
      ...
      this.$axios
        .post(
          `/management/admin/beauty-appraisal!sticky.action?id=${
            this.tableData[index].id
          }&sticky=${status}`
        )
        .then(res => {
    ...
    updataSort(index, row) {
      this.$axios
        .post(
          `/management/admin/beauty-appraisal!updateTime.action?id=${row.id}`
        )
        .then(res => {
          if (res.status == 200) {
    ...
    handleAddProduct() {
      ...
      this.$axios
        .post(
          `/management/admin/beauty-appraisal-product-relation!save.action?beautyAppraisalId=${
            this.productRowId
          }`,
    ...
    deleteProduct(index, row) {
      this.$confirm("确定删除产品么？").then(res => {
        this.$axios
          .get(
            `/management/admin/beauty-appraisal-product-relation!delete.action?id=${
              row.id
            }&beautyAppraisalId=${this.productRowId}`
          )
          .then(_ => {
            this.$message.success("删除成功");
            this.getProductData(
    ...
    handleEditProduct() {
      ...
      this.$axios
        .post(
          `/management/admin/beauty-appraisal-product-relation!save.action?id=${
            this.innerProductRowId
          }&beautyAppraisalId=${this.productRowId}`,
          this.$qs.stringify(tempObj)
        )
        .then(res => {
    ...
    addComment(index, row) {
      ...
      if(!this.checkComment2DialogVisible){
 ...
 // 回复评论（二级评论）
    ...
    handleAddComment() {
      ...
      if (this.isSecondComment) {
        ...
        this.$axios
          .post(
            `/management/admin/new-comment!save.action?beautyId=${
              this.commentRowId
            }&id=${this.commentId}&type=2&parentId=${this.parentCommentId?this.parentCommentId:this.commentId}`,
        ...
        this.$axios
          ...
          .then(res => {
            this.$message.success("添加成功！");
            this.getCommentData(1,10,this.commentRowId)
            this.addCommentForm = {};
          })
          .catch(e => {
            this.$message.error("出了点问题-.-！");
    ...
    changeCommentPage2(val) {
      this.commentPage = val;
      this.getCommentData2(val, this.commentRow, this.commentRowId);
    },
    changeCommentSize2(val) {
      this.commentRow = val;
      this.getCommentData2(this.commentPage, val, this.commentRowId);
    },
    // 产品分页
    ...
    // 评论数据
   getCommentData(page, rows, id) {
      this.$axios
        .post(`/management/admin/new-comment!list.action?beautyId=${id}&type=2`, this.$qs.stringify({page,rows}))
        .then(res => {
          if (res.data.total > 0) {
    ...
    getCommentData2(page, row, id) {
      this.$axios.get(`/management/admin/new-comment!getCommentListByCommentId.action?id=${this.commentId}&beautyId=${this.userId}&type=2`,{params: {
            page: page,
            rows: row
          }}).then(res=>{
        if(res.status==200){
    ...
    getProductData(page, row, id) {
      this.$axios
        .get(
          `/management/admin/beauty-appraisal-product-relation!list.action?beautyAppraisalId=${id}`,
          {
            params: {
              page: page,
              rows: row
