<template>
  <div style="height: 100%">
    <el-container style="height: 100%;background: linear-gradient(120deg, #fff, #5983e8);">
      <el-header>
        <img
          src="../../images/icons/logo_home.png"
          alt
        >
        <span class="menuBtn">
          <i
      ...
      <el-container style="width: 100%;">
        <aside class="el-aside">
          ...
          <div class="asideItem">
            <p class="itemTitle">
              <i class="iconfont el-icon-my-meizhuang2"></i>
              <span v-if="!isCollapse">美妆镜APP</span>
            </p>
            <el-menu
            ...
            >
              <el-submenu
                ...
                v-for="(item) in tempMenulist[0].menus.filter((val)=>{return val.menus.length > 0})"
                :key="item.menuid"
                class="menuOut"
              >
                <template slot="title">
                ...
                <el-menu-item
                  :index="itemMenu.enname"
                  v-for="(itemMenu) in item.menus.filter((val)=>{return val.menus.length == 0})"
                  :key="itemMenu.menuid"
                  :class="$route.path=='/'+itemMenu.enname?'is-active':''"
                >
                  <span style="font-size: 14px;">{{itemMenu.menuname}}</span>
                </el-menu-item>
                <el-submenu
                  :index="(itemMenu2.menuid).toString()"
                  v-for="(itemMenu2) in item.menus.filter((val)=>{return val.menus.length > 0})"
                  :key="itemMenu2.menuid"
                >
                  <template slot="title">
                    <i
                      :class="itemMenu2.icon"
                      class="icon iconfont"
                    ></i>
                    <span
                      slot="title"
                      style="font-size: 14px;"
                    >{{itemMenu2.menuname}}</span>
                  </template>
                  <el-menu-item
                    :index="k.enname"
                    v-for="(k) in itemMenu2.menus"
                    :key="k.menuid"
                    :class="$route.path=='/'+k.enname?'is-active':''"
              ...
              <el-menu-item
                ...
                v-for="(item) in tempMenulist[0].menus.filter((val)=>{return val.menus.length == 0})"
                :key="item.menuid"
                :class="$route.path=='/'+item.enname?'is-active':''"
                class="singleTag"
              >
                <i
                  :class="item.icon"
          ...
          <div class="asideItem">
            ...
            >
              <el-submenu
                :index="(item.menuid).toString()"
                v-for="(item) in tempMenulist[1].menus.filter((val)=>{return val.menus.length > 0})"
                :key="item.menuid"
                class="menuOut"
              >
                <template slot="title">
                ...
                >
                  ...
                  <el-menu-item
                    ...
                    :class="$route.path=='/'+k.enname?'is-active':''"
                  >
                    <span style="text-align: right;">{{k.menuname}}</span>
                  </el-menu-item>
                </el-submenu>
              </el-submenu>
              <el-menu-item
          ...
          <div class="asideItem">
            <p class="itemTitle">
              <i class="iconfont el-icon-my-meizhuang2"></i>
              <span v-if="!isCollapse">美妆镜硬件</span>
            </p>
            <el-menu
              class="el-menu-vertical-demo"
              text-color="#3F51B5"
            ...
            >
              <el-submenu
                :index="(item.menuid).toString()"
                v-for="(item) in tempMenulist[2].menus.filter((val)=>{return val.menus.length > 0})"
                :key="item.menuid"
                class="menuOut"
              >
                <template slot="title">
...
export default {
  ...
  methods: {
    getPath(path) {
      console.log(path);
      this.$store.commit("getRoute", path);
      if (path == "quickWork") {
