    ...
    <div class="handle-box">
      ...
      <el-dialog title="新增" :visible.sync="AddVisible" width="80%" @opened="addOPen">
        ...
        <el-form-item label="搜索标签" label-width="120px">
          ...
          <el-form-item label="眉形描述" label-width="120px">
            <textarea id="hairDescAdd" rows="10" cols="80"></textarea>
          </el-form-item>
          <el-form-item label="眉形图片" label-width="120px">
            <el-upload
              class="upload-demo"
          ...
          <el-form-item label="教程id" label-width="120px">
              <el-input type="text" v-model="formAdd.beautyDetailsId"></el-input>
          </el-form-item>
    ...
    <el-dialog title="编辑" :visible.sync="TableVisible" width="80%">
      <el-form :model="formEdit">
        ...
        <el-form-item label="眉形图片" label-width="120px">
          ...
          <el-form-item label="教程id" label-width="120px">
              <el-input type="text" v-model="formEdit.beautyDetailsId"></el-input>
          </el-form-item>
           <el-form-item label="搜索产品" label-width="120px">
            <el-input v-model="searchBeautiColor" @input="getBeautiColorList" clearable></el-input>
            <el-table
...
export default {
  ...
  methods: {
    getlabelCountList(page1, row1) {
      this.$axios.post("/management/admin/brow!list.action", this.$qs.stringify({
            page: page1,
            row: row1
          })).then(res => {
        if (res.status == 200) {
    ...
    getlabelList() {
      this.$axios
        .get(`/management/admin/label!getTreeGrid.action?id=811`)
        .then(res => {
          if (res.status == 200) {
              console.log(res)
 ...
 this.choosedLabelsList.splice(val, 1);
    ...
    addOPen() {
      ...
      this.formAdd.hairstyleDescribe = CKEDITOR.instances.hairDescAdd.setData();
    },
    //新增
    handleAdd() {
      this.formAdd.browDescribe = CKEDITOR.instances.hairDescAdd.getData();
      ...
      this.$axios
        .post(
          `/management/admin/brow!save.action`,
    ...
    handleEdit(row) {
      ...
      this.idx = row.id;
      this.$axios
        .get(`/management/admin/brow!input.action?id=${this.idx}`)
        .then(res => {
          if (res.status == 200) {
            console.log(res);
            this.formEdit = res.data;
            this.formEdit.browImage?this.fileList1= [{url:res.data.browImage,name:'眉形'}]:this.fileList1=[]
            this.formEdit.eyebrowShareImage?this.shareList= [{url:res.data.eyebrowShareImage,name:'眉形分享'}]:this.shareList=[]
      ...
      setTimeout(() => {
        ...
        CKEDITOR.instances.hairDesc.setData(row.browDescribe);
      }, 10);
    },
    saveEdit() {
      this.formEdit.browDescribe = CKEDITOR.instances.hairDesc.getData();
    ...
    deleteRow(index, rows) {
      this.$confirm("此操作将永久删除该数据, 是否继续?", "提示", {
        ...
        this.$axios
          .get(`/management/admin/brow!delete.action?id=${rows[index].id}`)
          .then(res => {
            if (res.status == 200) {
   ...
   handlePreview(file) {
    ...
    shareSuccess(res) {
      this.formEdit.eyebrowShareImage = res.url;
      if (this.AddVisible) {
        this.formAdd.eyebrowShareImage = res.url;
      }
    },
    //分页
