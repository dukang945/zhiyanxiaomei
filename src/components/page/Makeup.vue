				 ...
				 size="mini">
					...
					<el-form-item label="品牌" prop="brandId">
						<el-select v-model="formLabelAdd.brandId" filterable clearable placeholder="请选择品牌">
							<el-option v-for="item in gridList" :key="item.id" :label="item.name" :value="item.id"></el-option>
							<el-pagination @size-change="changeSize4" @current-change="changePage4" :current-page="page4" :page-sizes="[10, 20]"
							 :page-size="10" layout="total, sizes, prev, pager, next, jumper" :total="totalNum4"></el-pagination>
						</el-select>
					</el-form-item>
					...
					<el-form-item label="分类">
						<el-select v-model="formLabelAdd.categoryId" filterable clearable :change="change" placeholder="请选择分类">
							<el-option v-for="(item,index) in categoryList" :key="index" :label="item.name" :value="item.id"></el-option>
							<el-pagination @size-change="changeSize5" @current-change="changePage5" :current-page="page5" :page-sizes="[10, 20]"
							 :page-size="10" layout="total, sizes, prev, pager, next, jumper" :total="totalNum5"></el-pagination>
						</el-select>
					</el-form-item>
					...
					<el-form-item label="功效">
						<el-select v-model="formLabelAdd.gx" filterable clearable multiple placeholder="请选择功效">
							...
							<el-pagination @size-change="changeSize6" @current-change="changePage6" :current-page="page6" :page-sizes="[10, 20]"
							 :page-size="10" layout="total, sizes, prev, pager, next, jumper" :total="totalNum6"></el-pagination>
						</el-select>
					</el-form-item>
					...
					<el-form-item label="图片">
						<el-upload class="upload-demo" action="management/admin/kcupload!uploadImage.action" :data="imgData1" :on-preview="handlePreview"
						 :on-remove="handleRemove1" :on-success="handleSuccess1" :file-list="fileList" :before-upload="beforeUpload1"
						 list-type="picture">
		...
		<el-table :data="makeupList" border style="width: 100%">
			...
			<el-table-column label="操作" align="center">
				<template slot-scope="scope">
					<el-button @click.native.prevent="deleteRow(scope.$index, makeupList)" type="danger" size="small" class="el-icon-delete"
					 v-del>删除</el-button>
					...
					<el-dialog title="编辑" :visible.sync="dialogVisible" :before-close="handleClose">
						<el-form :label-position="labelPosition" label-width="120px" :model="formLabelAlign" :rules="rules" ref="formLabelAlign"
						 size="mini">
							...
							<el-form-item label="品牌" prop="brandId">
								<el-select v-model="formLabelAlign.brandId" filterable clearable placeholder="请选择品牌">
									<el-option v-for="item in gridList" :key="item.id" :label="item.name" :value="item.id"></el-option>
									<el-pagination @size-change="changeSize4" @current-change="changePage4" :current-page="page4" :page-sizes="[10, 20]"
									 :page-size="10" layout="total, sizes, prev, pager, next, jumper" :total="totalNum4"></el-pagination>
								</el-select>
							...
							<el-form-item label="分类">
								<el-select v-model="formLabelAlign.categoryId" filterable clearable placeholder="请选择分类">
									<el-option v-for="(item,index) in categoryList" :key="index" :label="item.name" :value="item.id"></el-option>
									<el-pagination @size-change="changeSize5" @current-change="changePage5" :current-page="page5" :page-sizes="[10, 20]"
									 :page-size="10" layout="total, sizes, prev, pager, next, jumper" :total="totalNum5"></el-pagination>
								</el-select>
							...
							<el-form-item label="功效">
								<el-select v-model="formLabelAlign.gx" filterable clearable multiple placeholder="请选择功效">
									<el-option v-for="(item,index) in problemList" :key="index" :label="item.name" :value="item.id"></el-option>
									<el-pagination @size-change="changeSize6" @current-change="changePage6" :current-page="page6" :page-sizes="[10, 20]"
									 :page-size="10" layout="total, sizes, prev, pager, next, jumper" :total="totalNum6"></el-pagination>
								</el-select>
							...
							<el-form-item label="图片">
								<el-upload class="upload-demo" action="management/admin/kcupload!uploadImage.action" :data="imgData"
								 :on-preview="handlePreview" :on-remove="handleRemove" :on-success="handleSuccess" :file-list="fileList2"
								 :limit="1" :before-upload="beforeUpload" list-type="picture">
									<el-button size="small" type="primary">点击上传</el-button>
									<div slot="tip" class="el-upload__tip">只能上传jpg/png文件，且不超过500kb</div>
								</el-upload>
							</el-form-item>
