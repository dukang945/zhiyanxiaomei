<template>
	<div>
		...
		<el-table :data="columnList" border style="width: 100%" @selection-change="handleSelectionChange">
			...
			<el-table-column label="操作" align="center">
				<template slot-scope="scope">
					<el-button @click.native.prevent="deleteRow(scope.$index, columnList)" type="danger" size="small" class="el-icon-delete">删除</el-button>
		...
		<el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="currentPage4"
		 :page-size="10" layout="total, sizes, prev, pager, next, jumper" :total="columnList.length"></el-pagination>
	</div>
</template>
...
<script>
	export default {
		...
		methods: {
			...
			//保存编辑
			saveEdit() {
				this.$set(this.columnList, this.idx, this.formLabelAlign);
			...
			deleteRow(index, rows) {
				...
				this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
					...
					this.$axios.get(`/management/admin/skills!delete.action?id=${rows[index].id}`).then(
						res => {
							if (res.status == 200) {
