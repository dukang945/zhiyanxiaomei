    ...
    <div class="handle-box">
      ...
      <el-dialog title="新增" :visible.sync="AddVisible" width="80%" @opened="addOPen">
        <el-form :model="formAdd">
          ...
          <el-form-item label="发型描述" label-width="120px">
            <textarea id="hairDescAdd" rows="10" cols="80"></textarea>
          </el-form-item>
          <el-form-item label="发型图片" label-width="120px">
            <el-upload
              class="upload-demo"
...
export default {
  ...
  methods: {
    getlabelCountList(page1, row1) {
      this.$axios.post("/management/admin/hairstyle!list.action", this.$qs.stringify({
            page: page1,
            row: row1
          })).then(res => {
        if (res.status == 200) {
 ...
 this.choosedLabelsList.splice(val, 1);
    ...
    //新增
    handleAdd() {
      this.formAdd.hairstyleDescribe = CKEDITOR.instances.hairDescAdd.getData();
      ...
      this.$axios
        .post(
          `/management/admin/hairstyle!save.action`,
    ...
    handleEdit(row) {
      ...
      this.idx = row.id;
      this.$axios
        .get(`/management/admin/hairstyle!input.action?id=${this.idx}`)
        .then(res => {
          if (res.status == 200) {
            console.log(res);
            this.formEdit = res.data;
            this.fileList1= [{url:res.data.hairstyle,name:'发型'}]
      ...
      setTimeout(() => {
        ...
        CKEDITOR.instances.hairDesc.setData(row.hairstyleDescribe);
      }, 10);
    },
    saveEdit() {
      this.formEdit.hairstyleDescribe = CKEDITOR.instances.hairDesc.getData();
      ...
      this.$axios
        .post(
          `/management/admin/hairstyle!save.action?id=${this.idx}`,
          this.$qs.stringify({
            hairstyleDescribe:this.formEdit.hairstyleDescribe,
            hairstyle:this.formEdit.hairstyle,
          }) +labels
        )
        .then(res => {
          if (res.status == 200) {
    ...
    deleteRow(index, rows) {
      this.$confirm("此操作将永久删除该数据, 是否继续?", "提示", {
        ...
        this.$axios
          .get(`/management/admin/hairstyle!delete.action?id=${rows[index].id}`)
          .then(res => {
            if (res.status == 200) {
    ...
    handleSuccess1(res) {
      this.formEdit.hairstyle = res.url;
      if (this.AddVisible) {
        this.formAdd.hairstyle = res.url;
      }
    },
   handlePreview(file) {
      this.img = file.url;
