                        ...
                        >
                          ...
                          <el-form-item label="小步骤说明">
                            <el-input
                              ...
                              style="width: 80%;"
                              autosize
                              v-model="formLabelAdd.beautyDetailsRelationList[index].childrenProcedureList[key].procedureDes"
                            ></el-input>
                          </el-form-item>
      ...
      <el-form :label-position="labelPosition" label-width="120px" :model="editFormData">
        ...
        <el-form-item label="化妆步骤">
          <el-tabs
            ...
            type="border-card"
            editable
            @tab-remove="removeEditTab"
            @tab-add="editAddMainStep"
          >
            <el-tab-pane
              v-for="(item, index) in stepEditList"
              :key="item.id"
              :label="item.name"
              :name="item.name"
            >
                  ...
                  >
                    ...
                    <el-form-item label="小步骤说明">
                      <el-input
                        ...
                        style="width: 80%;"
                        autosize
                        v-model="editFormData.beautyDetailsRelationList[index].childrenProcedureList[key].procedureDes"
                      ></el-input>
                    </el-form-item>
                    <el-form-item label="小步骤图片">
                      <el-upload
    ...
    <el-dialog :visible.sync="H5PageDialogVisible" width="70%" top="20px" @close="colseViewPage">
      <div class="content">
        ...
        <div class="pageContent">
          ...
          <div id="time">
            <span>{{getTime(1,1,viewPageData.createTime).split(" ")[0]}}</span>
            <span>视频来源：{{viewPageData.source?viewPageData.source:'原创'}}</span>
          </div>
          <div class="rate">
            <p>难度</p>
            <ul>
              <li v-for="item in viewPageData.star">
          ...
          <div id="label">
            <div
              id="labelLeft"
              v-for="(item) in (viewPageData.labelName?viewPageData.labelName.split(','):[])"
            >
          ...
          <div v-for="(item,index) in viewPageData.beautyDetailsRelationList">
            ...
            <div
              class="intro"
              v-for="(childrenItem,key) in item.childrenProcedureList"
              :key="childrenItem.id"
              v-if="item.procedureName!='结语'"
            >
              <div
                class="sequence"
                v-if="item.childrenProcedureList.length>1&&childrenItem.desSort!=98&&childrenItem.desSort!=99"
              >
                <div class="sequence1"></div>
  ...
  methods: {
    ...
    saveDraft() {
      ...
      for (let i = 0; i < tempObj.beautyDetailsRelationList.length; i++) {
        if (
          tempObj.beautyDetailsRelationList[i].childrenProcedureList.length == 1
        ) {
          tempObj.beautyDetailsRelationList[
            i
          ].childrenProcedureList[0].desSort = "";
        }
      }
      tempObj.beautyDetailsRelationList = JSON.stringify(
        tempObj.beautyDetailsRelationList
      );
      let labelStr = "";
      let colorStr = "";
      ...
      this.$axios
        .post(
          "/management/admin/beauty-details!save.action",
          this.$qs.stringify(tempObj) + colorStr + labelStr
        )
        .then(res => {
          if (res.data) {
    ...
    handleAdd() {
      ...
      if (this.formLabelAdd.productColor.length > 0) {
        for (let i = 0; i < this.formLabelAdd.productColor.length; i++) {
          productColorString += `&productColor=${
            this.formLabelAdd.productColor[i]
          }`;
        }
      }
      var labelIdString = "";
      ...
      let paramsStr =
        this.$qs.stringify(testObj) + productColorString + labelIdString;
      this.$axios
        .post("/management/admin/beauty-details!save.action", paramsStr)
        .then(res => {
          this.getTableData(
            "/management/admin/beauty-details!list.action",
            this.page,
    ...
    batchOnline() {
      if (this.checkedRowId) {
        this.$axios
          .post(
            `/management/admin/beauty-details!batchOnLine.action?ids=${
              this.checkedRowId
            }&online=0`
          )
          .then(res => {
    ...
    batchOffline() {
      if (this.checkedRowId) {
        this.$axios
          .post(
            `/management/admin/beauty-details!batchOffLine.action?ids=${
              this.checkedRowId
            }&online=1`
          )
          .then(res => {
    ...
    batchDelete() {
      if (this.checkedRowId) {
        this.$confirm("确定要进行批量删除么？").then(res => {
          this.$axios
            .post(
              `/management/admin/beauty-details!batchDelete.action?ids=${
                this.checkedRowId
              }`
            )
            .then(res => {
    ...
    freshH5() {
      this.$axios
        .get("/management/admin/beauty-details!refreshHTML.action")
        .then(res => {
          console.log(res);
        });
    },
    ...
    addLabel() {
      if (this.choosedLabelList.length > 0) {
        ...
        this.$axios
          .post(
            `/management/admin/beauty-details!batchAddLabel.action?ids=${
              this.checkedRowId
            }`,
            idString
          )
    ...
    edit(index, row) {
      ...
      that.$axios
        .get(
          `/management/admin/beauty-color!getSelectDetail.action?id=${row.id}`
        )
        .then(res => {
    ...
    deleteRow(index, row) {
      this.$confirm("确认删除？")
        .then(_ => {
          this.$axios
            .get(`/management/admin/beauty-details!delete.action?id=${row.id}`)
            .then(res => {
    ...
    switchOnline(index) {
      ...
      this.$axios
        .post(
          `/management/admin/beauty-details!online.action?id=${
    ...
    updataSort(index, row) {
      this.$axios
        .post(`/management/admin/beauty-details!updateTime.action?id=${row.id}`)
        .then(res => {
          if (res.status == 200) {
    ...
    getModuleData() {
      this.$axios
        .get("/management/admin/beauty-module!getList.action")
        .then(res => {
    ...
    getStepData(page, row, id) {
      this.$axios
        .get(
          `/management/admin/beauty-details-relation!list.action?beautyDetailsId=${id}`,
          {
            params: {
              page: page,
              rows: row
    ...
    addMiniStep(index) {
      ...
      this.formLabelAdd.beautyDetailsRelationList[
        ...
        id:
          this.formLabelAdd.beautyDetailsRelationList[index]
            .childrenProcedureList.length + 1,
        procedureDes: "",
        beautyDetailsRelationId: 0,
        procedureImg: [],
        desSort:
    ...
    editAddMiniStep(index) {
      ...
      this.editFormData.beautyDetailsRelationList[
        ...
        id:
          this.editFormData.beautyDetailsRelationList[index]
            .childrenProcedureList.length + 1,
        procedureDes: "",
        beautyDetailsRelationId: 0,
        procedureImg: [],
...
.sequence1 {
  ...
  -webkit-box-shadow: -0.43px 0.43px 0px #b581ff inset,
    -0.43px 0.43px 0px #b581ff;
...
.angled {
  ...
  background-image: -webkit-gradient(
    ...
    0 100%,
    100% 0,
    color-stop(0.25, rgba(153, 153, 153, 0.2)),
    color-stop(0.25, transparent),
    color-stop(0.5, transparent),
    color-stop(0.5, rgba(153, 153, 153, 0.2)),
    color-stop(0.75, rgba(153, 153, 153, 0.2)),
    color-stop(0.75, transparent),
    to(transparent)
  );
  background-image: -moz-linear-gradient(
    45deg,
    rgba(153, 153, 153, 0.2) 25%,
    transparent 25%,
    transparent 50%,
    rgba(153, 153, 153, 0.2) 50%,
    rgba(153, 153, 153, 0.2) 75%,
    transparent 75%,
    transparent
  );
